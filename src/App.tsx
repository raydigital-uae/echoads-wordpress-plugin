...
export const AudioPlayer = ({  audioData, bgColor }: AudioPlayerProps) => {
  ...
  const { config, isLoading } = useConfig(audioData.configEndpoint, audioData.apiKey);
  ...
  const performStatusCheck = async () => {
    ...
    const result = await checkAudioStatus(audioData.statusEndpoint, audioData.apiKey);
  ...
  useEffect(() => {
    if (!isLoading && config && tracks.length > 0 && !showPlayer) {
      ...
      performStatusCheck().then((canPlayAudio) => {
        if (canPlayAudio) {
          // For autoplay, start playback muted to comply with
          // browser autoplay policies while still indicating
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">{tracks[currentTrack]?.name || 'Audio Player'}</span>
